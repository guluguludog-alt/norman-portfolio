...
const BEICHUAN_TEMP = [5.3, 7.6, 12.1, 17.2, 21.3, 24.1, 25.5, 25.1, 21.2, 16.5, 11.6, 6.8]; 
const BEICHUAN_RAIN = [12.5, 19.3, 35.1, 68.4, 122.3, 185.6, 320.4, 290.1, 145.2, 65.8, 22.1, 9.5]; 
const BEICHUAN_SUN = [65.2, 72.1, 102.4, 125.6, 132.1, 115.4, 142.3, 148.6, 95.2, 82.1, 75.4, 62.5]; 
...
export default function Project2() {
  ...
  return (
    /* 【黄金修复点】挂载 data-lenis-prevent 属性，硬核屏蔽全站 Lenis 平滑滑轮手势拦截，恢复极其顺畅的窄屏原生上下滚动 */
    <div 
      className={`project2-page ${isZh ? 'zh-mode' : 'en-mode'} ${isExiting ? 'is-exiting' : ''}`}
      data-lenis-prevent
    >
      ...
      <div className="project2-pill-tooltip" style={{ display: tooltip.display, left: tooltip.left, top: tooltip.top }}>
      ...
      <div className="dashboard-container">
        <div className="side-column column-left">
          <div className="panel panel-culture-gallery">
            ...
            <div className="panel-body">
              <div className={`gallery-stack-container ${isGalleryEntering ? 'gallery-entering' : ''}`}>
                <div className="gallery-stack-wrapper">
                  {cultureSlides.map((slide, idx) => {
                    let offset = idx - currentSlide;
                    if (offset < 0) offset += cultureSlides.length; 
                    
                    const stackIndex = (offset >= cultureSlides.length - 1) ? cultureSlides.length - 1 : offset;
                    const zIndex = cultureSlides.length - stackIndex;

                    return (
                      <div 
                        key={idx} 
                        data-stack-index={stackIndex}
                        className={`gallery-slide-item ${stackIndex === 0 ? 'active' : ''}`}
                        style={{ 
                ...
                <div className="gallery-indicators-bar">
                  {cultureSlides.map((_, idx) => (
                    <div 
                      key={idx} 
                      className={`indicator-line-segment ${idx === currentSlide ? 'active' : ''}`}
                      onClick={() => setCurrentSlide(idx)}
                    />
                  ))}
                </div>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        animation: `project2-scaleIn 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) ${delay}s forwards`
                      }}
        ...
        <div className="side-column column-right">
          ...
          <div className="panel panel-description">
            ...
            <div className="project2-description-body panel-body description-box fade-text">
